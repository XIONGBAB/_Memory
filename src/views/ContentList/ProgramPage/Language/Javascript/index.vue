<template>
      ...
      <el-card id="part1" shadow="hover">
        ...
        <div class="ls-cot">
          ...
          <ul>
            ...
            <li>js写代码，全部写在末尾，因为文档页面从上往下加载，如果需要在上面，需要加个load事件，等页面加载完了再加载js</li>
            <li>写代码的时候，注意 大小写 否则程序不执行，有些不报错，有些会报错</li>
  ...
  if (element) {
    ...
    console.error('Element not found');
          ...
          <ul>
            ...
            <li>浏览器本身并不会执行JS代码，而是通过内置JavaScript引擎（解释器）来执行JS代码。</li>
            <li>JS引擎执行代码时逐行解释。每一句源码（转换为机器语言），然后由计算机去执行，所以JavaScript 语言归为脚本语言，会逐行解释执行。</li>
            <li>ECMAScript是一个标准，而这个标准需要由各个厂商去实现。不同的浏览器厂商对该标准会有不同的实现。</li>
          </ul>
          <table>
            <tbody>
              <tr>
  ...
  js语法           页面文档对象模型      浏览器对象模型
          ...
          <ul>
            <li>ECMAScript是由ECMA国际（ECMA（Europe Computer Manufacure Association ）原欧洲计算机制造商联盟）进行标准化的一门编程语言，这种语言在万维网上应用广泛，它往往被称为JavaScript或JScript，但实际上后两者是ECMAScript语言的实现和扩展。，使用最多的是ECMAScript5（ES5），最新版本：ES6采用类似强类型。（因为javascript的网景公司捐献出来，由ECMA来统一标准，所以现在遵循的是ECMAS的语法规则，也简称 ES）ECMAScript: ECMAScript规定了JS的编程语法和基础核心知识，是所有浏览器厂商共同遵守的一套JS语法工业标准。</li>
            <li>DOM 文档对象模型（DocumentObject Model），是W3C组织推荐的处理可扩展标记语言的标准编程接口。通过DOM提供的接口可以对页面上的各种元素进行操作（大小、位置、颜色等）。</li>
            <li>BOM 浏览器对象模型（Browser Object Model，）是指浏览器对象模型，它提供了独立于内容的、可以与浏览器窗口进行互动的对象结构。通过BOM可以操作浏览器窗口，比如弹出框、控制浏览器跳转、获取分辨率等</li>
          </ul>
          <p>js书写方式(双标签)</p>
          <pre><code class="language-html line-numbers">  // 行内：
  ...
  &lt;script defer src="url"&gt;&lt;/script&gt;</code></pre>
          <ul>
            <li>必须嵌套在html文件中运行，可以嵌套在head标签里面，也可以嵌套在 body</li>
            <li>JavaScript的代码必须嵌套在&lt;script&gt;标签里面，由于现在的主流浏览器默认的脚本语言就是JavaScript，所以不写type类型也是可以的</li>
          </ul>
          <p>注释</p>
  ...
  */
      function add(a, b) {
          ...
          <p>js输入输出语句</p>
          <ul>
            <li>为了方便信息的输入输出，JS中提供了一些输入输出语句，其常用的语句如下：</li>
          </ul>
  ...
  console.error('a');        /* 输出错误信息 */</code></pre>
          ...
          <ul>
            ...
            <li>js严格区分大小写</li>
            <li>每条语句以分号结尾，不添加的话，浏览器会自动添加，但是会消耗浏览器的资源，有时会加错。</li>
            <li>js会忽略多个空格和换行</li>
          </ul>
          <p>控制台错误解释</p>
          <ul>
            <li>Uncaught ReferenceError: Alert is not defined // 引用错误，Alert 未定义</li>
        ...
        <div class="ls-cot">
          ...
          <ul>
            <li>
              声明变量： var age;
              <ol>
                <li>var: 是一个JS关键字，用来声明变量（variable 是变量的意思）。 使用该关键字声明变量后，计算机会自动为变量分配内存空间，不需要程序员管理</li>
          ...
          <p>更新变量</p>
          <ul>
            <li>一个变量被重新复赋值后，它原有的值就会被覆盖，变量值将以最后一次赋的值为准。</li>
          </ul>
  ...
  */
  ...
  // 5、对于数组和对象的元素修改，不算做对常量的修改，不会报错</code></pre>
          ...
          <ul>
            ...
            <li>let： 不会变量提升，不能重复声明，且块级作用域下可以重复声明，不影响外部let 声明的变量</li>
            <li>const： 不会提升，不可重复声明，不可赋值，常量在定义的时候必须要赋值，之后不可修改</li>
          </ul>
          <p>命名的规范</p>
          <ul>
            <li>由字母（A-Z a-z）、数字（0-9）、下划线（_）、美元符号（$）组成，如：usrAge，num01，name_1</li>
            <li>严格区分大小写。var app；和 var App；是两个变量</li>
            <li>不能以数字开头。18age是错误的</li>
            <li>不能是关键字、保留字。例如：var、for、while，this、name、string、number， 注意name是系统关键字，要避免使用</li>
  ...
  apple2 = temp;            // 最后把temp的值赋予给apple 2</code></pre>
          ...
          <ul>
            ...
            <li>
              变量怎么使用的？
              <ol>
                <li>我们使用变量的时候，一定要声明变量，然后赋值，声明变量本质是去内存申请空间。</li>
              </ol>
            </li>
            <li>
              什么是变量的初始化？
   ...
   */</code></pre>
            ...
            <li>
              数字型 Number： JavaScript数字类型既可以用来保存整数值，也可以保存小数（浮点数）。
              <pre><code class="language-js line-numbers">  var age = 21;        // 整数
                var Age = 21.3777;   // 小数，也称为浮点数</code></pre>
              <ul>
                <li>
                  数字型范围 （注意大小写）
  ...
  // 注意: JS可以用单引号嵌套双引号，或者用双引号嵌套单引号（外双内单，外单内双）</code></pre>
              <ul>
                ...
                <li>
                  字符串转义符：
                  <ol>
                    <li>要写到引号里面类似HTML里面的特殊字符，字符串中也有特殊字符，我们称之为转义符。</li>
  ...
  alert(strMsg.length);             // 显示15， 因为空格也算一个字符</code></pre>
                ...
                <li>
                  字符串的拼接 ： 数值相加，字符相连
                  <ol>
                    <li>多个字符串之间可以使用 + 进行拼接，其拼接方式为字符+任何类型=拼接之后的新字符串</li>
  ...
  var res2 = Boolean(arr);   /* true */</code></pre>
            ...
            <li>
              Undefined 和 Null
              <ul>
                <li>一个声明后没有被赋值的变量会有一个默认值undefined（如果进行相连 或者 相加时，注意结果）</li>
  ...
  // 3.二进制以0b开头，只是了解，兼容性差，不建议使用
      var num = 0b10;</code></pre>
          <p>获取检测变量的数据类型</p>
          <ul>
            <li>typeof 可用来获取检测变量的数据类型，返回的结果是一个字符串。最后一个 null 是对象</li>
            <li>
              prompt( ); 中取出来的值是字符型
  ...
  console.log(null);              // null</code></pre>
          ...
          <p>数据类型转换</p>
          <ul>
            <li>使用表单、prompt获取过来的数据默认是字符串类型的，此时就不能直接简单的进行加法运算， 而需要转换变量的数据类型。通俗来说，就是把一种数据类型的变量转换成另外一种数据类型。</li>
            <li>
        ...
        <div class="ls-cot">
          ...
          <ul>
            <li>
              标识符
              <ol>
                <li>标识（zhi）符：就是指开发人员为变量、属性、函数、参数取的名字。标识符不能是关键字或保留字。</li>
              </ol>
            </li>
            <li>
              关键字
              <ol>
                <li>关键字： 是指JS本身已经使用了的字，不能再用它们充当变量名、方法名。</li>
            ...
            <li>
              保留字
              <ol>
                <li>实际上就是预留的“关键字”，意思是现在虽然还不是关键字，但是未来可能会成为关键字， 同样不能使用它们当变量名或方法名。</li>
                <li>包括： boolean、byte、char、class、const、debugger、double、enum、export、extends、fimal、float、 goto、implements、import、int、interface、long、mative、package、private、protected、public、 short、static、super、synchronized、throws、transient、volatile等。</li>
              </ol>
            </li>
          </ul>
        </div>
            ...
            <li>
              ...
              <ul>
                <li>
                  注意：
                  <ol>
                    <li>运算符 + ： 任何值和NaN 相加，都为NaN 。不是number值的数字会转换为数字，和字符串+则会拼接</li>
                    <li>运算符 - ： 数字和任何相减，都会被转为数字，与 运算符 + 不同</li>
                  </ol>
                </li>
              </ul>
            </li>
  ...
  console.log(a % b); //3 如果 a > b ,值等于b</code></pre>
              <ul>
                <li>浮点数值的最高精度是17位小数，但在进行算术计算时其精确度远远不如整数。不要直接判断两个浮点数是否相等！</li>
                <li>1 + 2 *3 结果是7，注意算术运算符优先级，先乘除，后加减，有小括号先算小括号里面的</li>
                <li>闰年：能被400整除的 就可以用取模运算符</li>
                <li>浮点数进行加减运算的时候要注意，避免用浮点数进行运算，后面会用取整</li>
              </ul>
            </li>
  ...
  console.log('8、n2=' + n2);           // 18</code></pre>
            ...
            <li>
              ...
              <ul>
                ...
                <li>开发时，大多使用后置递增/减，并且代码独占一行，例如：num++；或者num--；</li>
              </ul>
            </li>
          </ol>
          <p>表达式和返回值</p>
          <ul>
            <li>表达式：是由数字、运算符、变量等以能求得数值的有意义排列方法所得的组合卡</li>
            <li>简单理解：是由数字、运算符、变量等组成的式子</li>
            <li>表达式最终都会有一个结果，返回给我们，我们称为返回值</li>
            <li>正常算式： 1 + 2 = 3 // 3就是返回的结果，也称为返回值</li>
            <li>程序表达式： 3 = 1 + 2 // 右边的表达式算出来的值，赋给左边 ，3就是返回值</li>
          </ul>
          <p>比较运算符</p>
          <ul>
            <li>比较运算符（关系运算符）是两个数据进行比较时所使用的运算符，</li>
            <li>比较运算后，会返回一个布尔值（true/false）作为比较运算的结果。</li>
            <li>&gt;= 和 &lt;= 例如： 10 &gt;= 5 只要10大于了5 ，那么也会返回true ，大于或者等于，就会true</li>
            <li>任何值和 NaN 做比较，都是false</li>
            <li>如果两侧都是字符串，则会比较 字符串的 Unicode 编码大小，如果字符串数量不一样，则会一位一位比</li>
            <li>
              例如
              <ol>
                <li>“abc” &lt; “b” 此时会第一位相比较，因为没有第二位，如果两侧都有三位，则一位一位比 第一位都一样，则比较下一位</li>
              </ol>
            </li>
            <li>
              小知识
  ...
  result:true                        result:false</code></pre>
            ...
            <li>
              || ：
              <ol>
                <li>两侧条件判断，只要有一个判断为true，则结果为true ，两侧为false，则返回false</li>
  ...
  result:true                        result:false</code></pre>
            ...
            <li>
              ...
              <ol>
                ...
                <li>逻辑非（！）也叫作取反符，用来取一个布尔值相反的值，如true的相反值是false</li>
                <li>！！ 可以取反两次</li>
              </ol>
            </li>
            <li>
            ...
            <li>
              ...
              <ul>
                <li>
                  ...
                  <ol>
                    <li>表达式1 && 表达式2</li>
                    <li>如果第一个表达式的值为真，第二个表达式也为真，则返回表达式2</li>
                    <li>如果第一个表达式的值为假，第二个表达式就不看了，直接返回表达式1</li>
                    <li>例子</li>
                  </ol>
  ...
  console.log(0 && 456);      // 左边的表达式，0是false，如果表达式1为false，则逻辑中断，返回第一个结果 0</code></pre>
                ...
                <li>
                  ...
                  <ol>
                    <li>表达式1 || 表达式2</li>
                    <li>如果第一个表达式的值为真，第二个表达式就不看了，则返回表达式1</li>
                    <li>如果第一个表达式的值为假，第二个表达式为真，则返回表达式2</li>
                    <li>例子</li>
                  </ol>
  ...
  8    逗号运算符     ，</code></pre>
          <ul>
            <li>注意： 一元运算符里面的逻辑非的优先级比或和非高，逻辑与比逻辑或优先级高，如果实在不知道优先级，直接拿括号（） 括起来要先算的值</li>
            <li>
              练习1
  ...
  console.log(d);                             // d = true ||  F    d= T    </code></pre>
      ...
      <el-card id="part7" shadow="hover">
        ...
        <div class="ls-cot">
          <p>前言</p>
          <ul>
            <li>在一个程序执行的过程中，各条代码的执行顺序对程序的结果是有直接影响的。</li>
            <li>很多时候我们要通过控制代码的执行顺序来实现我们要完成的功能。</li>
            <li>
              简单理解：
              <ol>
                <li>流程控制就是来控制我们的代码按照什么结构顺序来执行。流程控制主要有三种结构，分别是</li>
                <li>顺序结构、分支结构和循环结构，这三种结构代表三种代码执行的顺序。</li>
              </ol>
  ...
  顺序执行       条件执行         循环执行
    ...
    B           A     B          判断------|    条件成立 yes，继续循环
            ...
            <li>顺序结构：是程序中最简单、最基本的流程控制，它没有特定的语法结构 程序会按照代码的先后顺序，依次执行，程序中大多数的代码都是这样执行的。</li>
            <li>
              分支结构：由上到下执行代码的过程中，根据不同的条件，执行不同的路径代码（执行代码多选一的过程），从而得到不同的结果。 JS语言提供了两种分支结构语句，if switch
              <ol>
                <li>
                  if 双分支语句： 如果条件表达式结果为true，则执行代码1，如果为false，则跳过执行代码1，执行else后面的执行代码2。
      ...
      以上条件都不成立时 执行最后这条代码
            ...
            <li>
              ...
              <ol>
                <li>
                  switch 语句也是多分支语句也可以实现多选1，语法结构switch 转换、开关 case 小例子或者选项的意思
  ...
  // 当代码相同时候，可以省略重复代码，因为碰到break才会跳出循环，所以会执行下一个case</code></pre>
            ...
            <li>
              执行思路：
              <ol>
                <li>让表达式的值和case 的 value 相匹配，全等了，执行语句，如果都不全等，则执行default：</li>
                <li>switch语句也是多分支语句，它用于基于不同的条件来执行不同的代码。</li>
                <li>当要针对变量设置一系列的特定值的选项时，就可以使用switch。</li>
              </ol>
            </li>
            <li>
              注意
              <ol>
                ...
                <li>我们num的值 和 case里面的值相匹配的时候是 全等 必须是值和数据类型一致才可以， num === 1</li>
                <li>如果不写 break; 来退出循环语句，则会执行下面的代码。执行完所有没带 break 的 case 和最后的 default：</li>
              </ol>
            </li>
            <li>
              ...
              <ol>
                ...
                <li>switch..case 语句通常处理case为比较确定值的情况，而if..else..语句更加灵活，常用于范围判断（大于、等于某个范围）</li>
                <li>switch 语句进行条件判断后直接执行到程序的条件语句，效率更高。而if..else语句有几种条件，就得判断多</li>
  ...
  console.log(`${year}-${month}-${day}是全年的第${sum}天`);</code></pre>
          ...
          <ul>
            ...
            <li>
              如果条件表达式结果为真，则返回 表达式1 的值，如果条件表达式为假，则返回 表达式2 的值， 语法结构 条件表达式 ？ 表达式1：表达式2
              <pre><code class="language-js line-numbers">  var num = 10 ;
  ...
  alert(result);</code></pre>
      ...
      <el-card id="part8" shadow="hover">
        ...
        <div class="ls-cot">
          <p>可以重复执行某些代码</p>
          <ol>
            <li>在程序中，一组被重复执行的语句被称之为循环体，能否继续重复执行，取决于循环的终止条件。</li>
            <li>
  ...
  for (var num1 = 1; num1 &lt;= 1000; num1++) {console.log('1开始');}</code></pre>
            ...
            <li>
              ...
              <ul>
                ...
                <li>初始化变量就是用var声明的一个普通变量，通常用于作为计数器使用</li>
                <li>条件表达式就是用来决定每一次循环是否继续执行就是终止的条件</li>
                <li>操作表达式是每次循环最后执行的代码经常用于我们计数器变量进行更新（递增或者递减）</li>
                <li>代码体验我们重复打印100句你好</li>
                <li>
                  注意：循环执行代码顺序
                  <ol>
                    <li>先执行计数器变量 var num1 =1； 只执行一次，然后条件判断，num1 <=100；</li>
                    <li>条件判断为true，执行console.log</li>
                    <li>执行log</li>
                    <li>最后执行 num1++，此时的 num1++ 之后，num1 为 2</li>
                    <li>那么num1 会不会小于等于 100，如果为真，则继续 ++，如果为假，则循环结束</li>
                    <li>++ 是最后执行的</li>
...
for (let i = 100; i &lt;1000; i++) {
      ...
      i++;
            ...
            <li>
              ...
              <ul>
                ...
                <li>很多情况下，单层for循环并不能满足我们的需求，比如我们要打印一个5行5列的图形、打印一个倒直角三角形等， 此时就可以通过循环嵌套来实现。</li>
                <li>
  ...
  while (num &lt;= 100){
      ...
      num++;
            ...
            <li>
              do..while循环:执行思路跟while不同的地方在于do while先执行一次循环体在判断条件， 如果条件表达式结果为真，则继续执行循环体，否则退出循环
              ...
              <ul>
                <li>
                  ...
                  <ol>
                    ...
                    <li>如果是用来计次数，跟数字相关的，三者使用基本相同，但是我们更喜欢用for</li>
                    <li>while和do..while可以做更复杂的判断条件，比for 循环灵活一些</li>
                    <li>while和do..while执行顺序不一样，while先判断后执行，do..while先执行一次，再判断执行</li>
                    <li>实际工作中，我们更常用for循环语句，它写法更简洁直观，所以这个要重点学习</li>
                  </ol>
                </li>
                <li>
                  循环
  ...
  for (let i = 1; i &lt;= 5; i++) {       /* 定义每行打印的内容 */
    ...
    console.log(str);
          ...
          <p>断点调试</p>
          <ul>
            <li>浏览器 F12 &gt;&gt; 在Sources &gt;&gt; 代码页面，想看哪里执行过程 就在行号点下，下断点，然后在控制里面执行下一步或者上一步</li>
            <li>可以在右侧添加变量进行观察</li>
            <li>还可以使用debugger关键字进行代码断点，直接在需要断点的代码行后添加debugger关键字即可;</li>
          </ul>
          <p>continue 、break (跟循环搭配使用)</p>
          <ul>
            <li>continue 关键字用于立即跳出本次循环，继续下一次循环（本次循环体中，遇到continue后立马退出不执行，直接 i++</li>
            <li>
      ...
      for(var j = 1; j&lt;=i;j++){
          ...
          <ul>
            <li>
              标识符命名规范
              <ol>
                <li>变量、函数的命名必须要有意义， 变量的名称一般用名词，函数的名称一般用动词</li>
              </ol>
            </li>
            <li>
              操作符命名规范：
