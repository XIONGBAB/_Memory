<template>
  <card-container :data="data">
    ...
    <template #describe>
      <p>
        ...
        XML）是一种用于创建快速动态网页的技术。它通过在后台与服务器进行少量数据交换，使得网页能够在不重新加载整个页面的情况下，对网页的某部分进行更新。
      </p>
    </template>
    <template #content>
      <el-card id="part1" shadow="hover">
        ...
        <p>前言</p>
        <ul>
          <li>Asynchronous javaScript and xml : 异步 js 和 xml</li>
          <li>异步: 指的是并行执行</li>
          <li>同步: 指的是按顺序从上往下执行</li>
          <li>
            ajax
            的好处，可以在不刷新页面的时候，完成数据加载，产生局部刷新的效果，提高用户体验度
          </li>
          <li>
            最初使用xml标签存储数据，现在绝大部分用JSON格式的字符串
            <pre><code class="language-html line-numbers">  &lt;!-- a.JSON文件 --&gt;
  {
      "name": "张三",
      "age": 20,
      "gender": "男"
  ...
  &lt;body&gt;
      ...
      &lt;/script&gt; </code></pre>
          ...
          <li>
            AJAX步骤：总共4步
            <pre><code class="language-js line-numbers">  let xhr = new XMLHttpRequest(); // 1、创建xhr对象 （原生ajax对象）
  ...
  xhr.onreadystatechange = function () {  // 4、监听读取状态改变事件
      if (xhr.readyState === 4) {         // 判断是否请求完成
          ...
          <li>
            JSON
            <pre><code class="language-js line-numbers">  // 可以存储对象
  {
  ...
  [
      ...
      },
      {
        ...
        <ul>
          ...
          <li>GET 传参是拼接在URL里</li>
          <li>? 后面跟参数， 使用 & 符号拼接参数</li>
          <li>
            参数数据的上限是4MB（M = MB），
            假如需要上传音乐或者小视频，那么GET就不够用
          </li>
          <li>且数据不安全，会以明文方式出现在URL上</li>
          <li>行内规范：一般只会请求数据，不会发送数据</li>
          <li>xhr.send(loginId=abc&Pwd=abc);</li>
          <li>POST 是将数据放在 send() 里 &拼接参数</li>
          <li>
            默认传参方式是text 文本类型
            ，在发送前需要设置请求头里的属性，Content-Type 百度查询类型
            <pre><code class="language-js line-numbers">  xhr.setRequestHeader('Content-Type','application/x-www-form-urlencoded');
  xhr.send(loginId=abc&Pwd=abc);

  // 类型为json时的写法
  xhr.setRequestHeader('Content-Type','application/json');
  xhr.send(`{"name":"张三","age":20,"gender":"男"}`);</code></pre>
          </li>
        </ul>
      </el-card>
    </template>
...
<script setup lang="ts" name="Ajax">
import { ref } from "vue";

const data = ref([{ id: 1, h2: "使用" }]);
</script>
