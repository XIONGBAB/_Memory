<template>
  <card-container :data="data">
    ...
    <template #describe>
      <p>
        MySQL是一个流行的关系型数据库管理系统（R
        DBMS），MySQL是一个开源的关系型数据库管理系统，它使用结构化的查询语言（SQL）进行数据管理。MySQL能够存储大量的数据，并提供数据的增加、删除、修改和查询功能。
      </p>
    </template>
    <template #content>
      <el-card id="part1" shadow="hover">
        ...
        <ul>
          ...
          <li>
            打开官网点DOWNLOADS => 点MySQL Community (GPL) Downloads » =>
            点MySQL Installer for Windows
          </li>
          <li>
            选系统，然后x64安装，点No thanks, just start my
            download.下载，32位是兼容64的
          </li>
          <li>
            双击安装 => 五种方式1默认 2服务器 3客户端 4插件 5自定义 => 默认next
            => 改路径 => 其他默认
          </li>
          <li>密码可设置大写 X-20220511</li>
          <li>
            卸载方法 => 打开安装包 => 点 remove => 全勾选 => 默认执行 =>
            应用卸载MySQL下载器
          </li>
          <li>
            文件安装方法： 解压 => 开后在bin|docs文件夹同级目录创建一个
            my.ini文件,并复制粘贴进去，改两行安装存放目录
          </li>
          <li>
            参数
            <ul>
              ...
              <li>max_connect_errors=10</li>
              <li>#服务端使用的字符集默认为UTF8</li>
              <li>character-set-server=utf8</li>
              <li>#创建新表时将使用的默认存储引擎</li>
              <li>default-storage-engine=INNODB</li>
              <li>#默认使用“mysql_native_password”插件认证</li>
              <li>default_authentication_plugin=mysql_native_password</li>
              <li>#设置mysql客户端默认字符集 default-character-set=utf8</li>
              <li>
                client #设置mysql客户端连接服务端时默认使用的端口 port=3306
                default-character-set=utf8
              </li>
              <li>
                去环境变量里添加路径,复制bin文件夹的路径即可,在电脑图标右键属性=>高级设置
                =>系统的环境变量=>找到PATH路径中添加bin的路径=>确认
              </li>
            </ul>
          </li>
          <li>打开终端或命令行 输入 mysql -uroot -p 测试安装成功与否</li>
          <li>
            连接mysql时报错：Public Key Retrieval is not
            allowed（不允许公钥检索）,解决方法：
            <ul>
              <li>
                右键项目，编辑 --
              ...
              <li>
                （这里的运输公钥检索是默认关闭的，需要把它开启），改为allowPublicKeyRetrieval=true即可。
              </li>
              <li>
        ...
        <table>
          <tbody>
            ...
            <tr>
              ...
              <td>O</td>
              <td>O</td>
              <td><a href="https://dbeaver.io/">https://dbeaver.io/</a></td>
            </tr>
            <tr>
              <td>navicat</td>
              <td></td>
              ...
              <td>O</td>
              <td>
                <a href="https://www.navicat.com.cn/"
                  >https://www.navicat.com.cn/</a
                >
              </td>
            </tr>
            <tr>
            ...
            <tr>
              ...
              <td></td>
              <td>
                <a href="https://www.devart.com/dbforge/mysql/studio/"
                  >https://www.devart.com/dbforge/mysql/studio/</a
                >
              </td>
            </tr>
          </tbody>
        ...
        <ul>
          <li>
            推荐总结:免费版本选择跨平台的 dbeaver (opens new
            window)，但如果你不差钱就选 navicat
          </li>
        </ul>
      </el-card>
      <el-card id="part2" shadow="hover">
        <h2>语法</h2>
        <p>连接 命令行模式</p>
        <pre><code class="language-js line-numbers">  mysql -uroot -p:                       // mysql命令   -uroot软件帐号   -p密码
  ...
  exit                                   // 所有的操作工作完成后，输入exit 进行结束，退出即可</code></pre>
        <p>创建/查看数据库</p>
        <pre><code class="language-js line-numbers">  create database web charset utf8;     // create database + 语义化数据库名 + 字符集;
  show databases;                       // 来查看数据库
  use web;                              // 进到文件夹内
  drop database web;                    // 删除数据库
  drop database if exists web;          // 如果数据库存在则删除，用 if exists 来判断
  show create database web;             // 查看数据库的结构</code></pre>
        <p>数据库的连接</p>
        <ul>
          <li>左上角+号点击 => 选 Popular 里面的MySQL</li>
          <li>输入数据库名称 + 密码 点下一步创建，驱动下载，完成</li>
          <li>
            项目展开=> 数据库右键=> 新建数据库=> 输入数据库名=> 选好字符集utf8=>
            排序选utf8_general_ci
          </li>
          <li>
            双击创建好的数据库，打开图形化界面 => 在工具栏找到SQL=> 下拉找到
            新建SQL编辑器
          </li>
          <li>
            输入语句按执行就可以看到数据库界面了，例如 show
            DATABASEs;点执行，下方就会出现数据库表(注意大小写)
          </li>
          <li>用完了可以用ctrl + / 注释掉 ，注意，每次执行都要加上分号 ；</li>
          <li>
            创建后的东西都要点击刷新，这样才会出来
            <pre><code class="language-js line-numbers">  CREATE TABLE class (id int PRIMARY KEY AUTO_INCREMENT,cname varchar(30) not null,description varchar(100) null) charset utf8;
  ...
  CREATE TABLE b (id int PRIMARY KEY AUTO_INCREMENT,name varchar(30)) select cname as name from class;  // 改名用 对应上name 旧名 as 新名 </code></pre>
          </li>
        </ul>
        <p>数据库查询</p>
        <pre><code class="language-js line-numbers">  // 查询
  SELECT                                                         // 查询
  ...
  INSERT INTO class (cname,description) VALUES('jsp','jsp学习'),('vue','学习vue'); // 多数据输入</code></pre>
        <p>数据维护</p>
        <pre><code class="language-js line-numbers">  ALTER TABLE class RENAME newClass;                             // 更改数据表的名字
  ...
  ALTER table class MODIFY id int not null AUTO_INCREMENT;       //  添加自增列</code></pre>
        <p>数据类型</p>
        <ul>
          <li>
            下列为支持的类型
          ...
          <li>字符集:数据库 &lt;= 表 &lt;= 字段 依次会继承</li>
          <li>show CHARACTED set; 显示数据库能使用的字符集</li>
          <li>校对规则： 是影响字符串之间的比较和排序的一种规则算法</li>
          <li>select * from class; 查看表</li>
          <li>
            select left(canme,2),right(canme,2),mid(canme,2,1),mid(canm,2) from
            class;
            <ul>
              <li>left()从左边开始取两个字符串 right()从右取</li>
              <li>
                mid()从任何位置,从第二个开始，取1个
                mid()任何位置,从第二个开始取所有
              </li>
            </ul>
          </li>
          <li>
            UPDATE class set cname = CONCAT('https://baidu,com',mid(cname,10))
            where id>=10; 设置内容并连接
          </li>
          <li>
            select SUBSTRING(cname,2) from class;
            跟mid()功能差不多，从什么开始，取后面的所有
          </li>
          <li>select CHAR_LENGTH(cname) from class; 计算字符串长度；</li>
          <li>
            select concat('编号'，id) as id ,concat('班级'，cname)as cname from
            calss; 修改并连接
          </li>
          <li>
            select if(CHAR_LENGTH(cname)>8,concat(left(cname,8),'...'),cname) as
            cname from class;
            <ul>
              <li>
                （如果字符长度大于8，那么就取8个字符和...连接，如果不超过就用cname)
                别名cnamse from class;）
              </li>
            </ul>
          </li>
          ...
          <li>
            ...
            REPLACE(cname,cname,CONCAT('后蹲人'，cname))
          </li>
          <li>where description REGEXP 'php|mysql'; 替换</li>
          <li>
          ...
          <li>
            ...
            <ul>
              <li>
                alter table class add status TINYINT; 添加status字段
                为tinyint类型的；值在-128 ~ 127之前
              </li>
              <li>
              ...
              <li>
                alter table class add status int(5) ZEROFILL; 设置显示的位数为5
                ，加zerofull会有前挡文填充 00
              </li>
            </ul>
            <table>
              <tbody>
                ...
                <tr>
                  ...
                  <td>4 字节</td>
                  <td>
                    (-3.402 823 466 E+38，-1.175 494 351 E-38)，0，(1.175 494
                ...
                <tr>
                  ...
                  <td>
                    ...
                    E-308)，0，(2.225 073 858 507 201 4 E-308，1.797 693 134 862
                    315 7 E+308)
                  </td>
                  <td>
                ...
                <tr>
                  <td>DECIMAL</td>
                  <td>DECIMAL(M,D) ，m&lt;65 是总个数，d &lt;30</td>
                  <td>依赖于M和D的值</td>
                  <td>依赖于M和D的值</td>
                </tr>
              </tbody>
          ...
          <li>
            ...
            <ul>
              <li>
                ...
                10代表整数部分和小数部分加起来总共10位，2代表有两位小数，6以下精准
              </li>
              <li>
                alter table class add b decimal(10,2);
                货币商品价格用decimal类型，其他类型用float
              </li>
            </ul>
          </li>
          <li>
            ...
            <ul>
              <li>
                ALTER TABLE class MODIFY sex ENUM('男'，'女') DEFAULT NULL；
                修改数据类型，1代表男 2代表女
              </li>
              <li>
                INSERT INTO class (cname,id,sex) VALUES('1'，'1','男')；
                在添加的时候直接设置也是可以的
              </li>
              <li>SELECT * FROM class WHERE sex = 2; 查找数据</li>
              <li>
                ALTER TABLE article ADD flag SET('推荐','置顶');
                多选项值,最多能写64个
              </li>
              <li>
                INSERT INTO article (title,status,click,flag)
                VALUES('标题','状态','点击数','推荐,置顶')； 添加数据
              </li>
              <li>
              ...
              <li>
                SELECT * FROM article WHERE flag LIKE '%推荐%'; 查找数据或者使用
                LIKE
              </li>
              <li>
                SELECT * FROM article WHERE flag = '推荐'; 查找数据
                需要完全匹配；
              </li>
              <li>
        ...
        <ul>
          <li>
            软件设置： 在DBeaver项目上右键 => 选择编辑 连接 =>
            服务器时区选Asia/ShangHai 或者自动检测
            <table>
              <tbody>
                <tr>
                  <th>日期时间类型</th>
          ...
          <li>
            ALTER TABLE class ADD birthday datetime default null;
            添加生日字段，类型为时间数据 默认为空
          </li>
          <li>
            UPDATE class set birthday ='1998-02-12 08:22:13' WHERE id=1;
            更新在id=2设置一个字段值
            <table>
              <tbody>
                <tr>
            ...
            <ul>
              <li>
                select cname,DATE_FORMAT(birthday,'%Y-%m-%d') from class;
                格式化日期第一为字段，第二为日期类型
              </li>
              <li>
                select cname,TIME_FORMAT(birthday,'%h:%i:%s') from class;
                以时间格式化，可以不写年月日
              </li>
              <li>
                select cname,TIME_FORMAT(birthday,'%h:%i:%s') from class;
              ...
              <li>
                ALTER TABLE class ADD updated timestamp DEFAULT
                CURRENT_TIMESTAMP; 添加字段 名更up.. 时间戳类型 默认值为当前时间
              </li>
              <li>
                ALTER TABLE class ADD updated timestamp DEFAULT
          ...
          <li>
            ...
            <ul>
              <li>
                select YEAR(birthday),from class; year()得到年 month()月 day()日
                时分秒的单词
              </li>
              <li>
                select now(),CURRENT_DATE()，CURRENT_TIME(); 取到当前的日期
                当前的时间
              </li>
              <li>
                select DAYOFYEAR(now()); 取到一年之中的第几天 DAYOFMONTH(now());
                星期日是1 - 星期六是7
              </li>
              <li>select WEEKDAY(now()); 星期一是0 - 星期日是6</li>
              <li>set @time = time(now()); 设置一个变量 设置为当前时间</li>
              <li>select @time; 输出显示</li>
              <li>
              ...
              <li>
                ...
                从元年到现在经过的天数, TO_DAYS返回是哪天
              </li>
              <li>
                select datediff(now(),barthday) form class; 两个时间相差的天数
              </li>
              <li>
                select timediff(time(now()),time(birthday)) form class;
                计算当前的时间和出生日期 两个时间差
              </li>
              <li>
                select timestampdiff(day,birthady,now())form class; 根据天
                单位来获取时间差
              </li>
              <li>
                select timestampdiff(month,birthady,now())form class;
                根据月/分/秒/周... 单位来获取时间差
              </li>
            </ul>
          </li>
          <li>
            ...
            <ul>
              <li>
                SELECT * from class WHERE birthday BETWEEN '1990-01-01' and
                '1999-12-31'; 介于之间的数据
              </li>
              <li>
                select * from class order by birthday desc limit 1;
                通过降序来查询，获取年龄最小的一位
              </li>
              <li>
                select * from class where birthday =(select birthday from class
          ...
          <li>
            一个大于20岁 的女生 最喜欢上的 班级 案例进行分写书写
            <ul>
              <li>select TIMESTAMPDIFF(YEAR,birthday,NOW()) from stu;</li>
              <li>select class_id FROM stu</li>
              <li>WHERE sex = 2</li>
              <li>and TIMESTAMPDIFF(YEAR,birthday,NOW()) > 20</li>
              <li>GROUP by class_id</li>
              <li>ORDER by count(id) DESC</li>
              <li>limit 1;</li>
            </ul>
          </li>
          <li>日期计算的写法</li>
          <ul>
            ...
            <li>
              ...
              得到当前时间后面增加8个小时后的结果,和上面一样
            </li>
            <li>
              SELECT date_add(now(),INTERVAL 7 DAY); 算出7天后的日期， -7是前7天
              ，day是单位 可以用年月日时分秒
            </li>
            <li>
            ...
            <li>
              SELECT date_sub(now(),INTERVAL "3 8" DAY); 反函数，往前减少，3天
              8小时之前
            </li>
            <li>SELECT last_day(now()); 返回当前月的最后一天</li>
            <li>
              SELECT date_sub(now(),INTERVAL DAYOFMONTH(now())-1 day); 获取月初
              -1是为了从1号月初开始计算
            </li>
            <li>SELECT * FROM article</li>
            <li>WHERE publish_time&lt;=last_day(now())</li>
            <li>
              AND publish_item>=date_sub(now(),INTERVAL DAYOFMONTH(now())-1
              day); 获取本月月末发表的文章写法
            </li>
            <li>SELECT * FROM article</li>
            <li>
              WHERE publish_item>=date_sub(now(),INTERVAL 3 MONTH);
              获取三个月内发表的文章，但并不是1号
            </li>
            <li>SELECT * FROM article</li>
            <li>
              WHERE publish_item>=DATE_FORMAT(date_sub(now(),INTERVAL 3
              MONTH),'%Y-%m-01); 这样才是前三个月1号开始
            </li>
            <li>
              SELECT last_day(date_sub(now(),interval 1 MONTH));
              取上个月最后一天
            </li>
            <li>SELECT last_day(now()); 取当月最后一天</li>
            <li>
              SELECT date_add(last_day(now()),interval 1 day)
              取当月最后一天并增加1天就是下月的1号；
            </li>
            <li>DAYOFWEEK 1-7 WEEKDAY 0-6 星期日 - 星期六 星期一 - 星期日</li>
            <li>
          ...
          <li>
            考勤表按月打卡查询 迟到
            <ul>
              <li>SELECT date_sub(now(),interval DATOFMONTH(now())-1 day);</li>
              <li>SELECT * FROM ATTENDANCE</li>
              <li>WHERE time(created_at)>'08:30:00'</li>
              <li>
                AND date(created-at)> date_sub(date_sub(now()),interval
                DATOFMONTH(now())-1 day);
              </li>
            </ul>
          </li>
          <li>
            考勤表按月打卡查询 迟到2次
            <ul>
              <li>SELECT stu_id,count(id) from attendance</li>
              <li>WHERE time(created_at)>'08:30:00'</li>
              <li>
                AND date(created-at)> date_sub(date_sub(now()),interval
                DATOFMONTH(now())-1 day)
              </li>
              <li>GROUP by stu_id</li>
              <li>HAVING count(id) >= 2;</li>
            </ul>
          </li>
          <li>
            考勤表按周打卡查询 迟到
            <ul>
              <li>SELECT date_add(now(),interval 0-WEEKDAY(NOW()) day);</li>
              <li>select * from attendance</li>
              <li>
                where date(created_at) >= date(date_add(now()interval
                0-WEEKDAT(now()) day))
              </li>
              <li>AND time(created_at) >= '09:00:00';</li>
            </ul>
          </li>
          <li>取到上周</li>
          <ul>
            <li>SET @week = date_sub(now(),interval 1 week);</li>
            <li>SELECT @week;</li>
            <li>select * from attendance</li>
            <li>
              where date(created_at)>date(date_add(@week,interval
              0-WEEKDAY(@week) day))
            </li>
            <li>
              and date(created_at) &lt;= date(date_add(@week,interval
              4-WEEKDAY(@week) day))
            </li>
            <li>and time(created_at) >'08:30:00';</li>
          </ul>
          <li>
            查看周日还打卡的
            <ul>
        ...
        <ul>
          <li>
            ...
            <ul>
              <li>
                SELECT * FROM class ORDRE BY sex ASC;
                升序，DESC降序，前提是拿到数据再排序
              </li>
              <li>
                SELECT * FROM class ORDRE BY id DESC LIMIT 1;
                取最新，第一个的数据
              </li>
              <li>
                SELECT * FROM class WHERE sex=2 ORDRE BY id DESC LIMIT 1;
                加入条件，取最新加入的女同学
              </li>
              <li>SELECT RAND(); 随机获取一个数据</li>
              <li>
                SELECT * FROM class ORDER BY RAND() DESC LIMIT 1;
                随机取一个数据值
              </li>
              <li>
                SELECT cname,birthday,MONTH(birthday) as m FROM stu ORDER BY m
                DESC; 按生日排序取数据
              </li>
              <li>
                SELECT field('a','a','b','c');
                a在后面三个数值中查找，返回在第几个位置出现， 1
              </li>
              <li>SELECT count(*) FROM stu; 会统计出目前数据库的数量</li>
              <li>
                SELECT count(*) FROM stu WHERE SEX='女'; 统计出女同学的数量 ，*
                代表所有字段
              </li>
              <li>
                SELECT count(class_id) FROM stu; 统计出女同学的数量
                ,指定字段，会排除为空null的数据
              </li>
              <li>
                SELECT class_id FROM stu WHERE class_id is not null ORDER BY
                class_id DESC LIMIT 1;
              </li>
              <li>
                SELECT max(class_id) from stu;
                取字段中最大的值，并过滤空数据，跟上面的作用一样
              </li>
              <li>SELECT min(class_id) from stu; 取字段中最小的值</li>
            </ul>
          </li>
          <li>
            求和 平均
            <ul>
              <li>SELECT sum(click) FROM article; 求和操作</li>
              <li>SELECT svg(click) FROM article; 平均操作</li>
              <li>
                SELECT sname,ROUND(AVG(timestampdiff(YEAR,birthday,now()))) FROM
                stu; 求学生的平均年龄
              </li>
            </ul>
          </li>
          <li>
            ...
            <ul>
              ...
              <li>
                SELECT DISTINCT class_id,name from stu WHERE class_id is not
                null; 当多个条件都满足才会去重
              </li>
            </ul>
          </li>
          <li>
            ...
            <ul>
              <li>
                SELECT class_id,count(*) from stu GROUNP BY class_id HAVING
                count(*)>=2; 人数超过2个同学的班级
              </li>
            </ul>
          </li>
...
<script setup lang="ts" name="=Mysql">
import { ref } from "vue";
